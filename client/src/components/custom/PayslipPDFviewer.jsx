...
export default function PayslipPDFViewer({ payslip = null, open, onClose }) {
  // PDF viewer state
  ...
  const handleOpenChange = (isOpen) => {
    if (isControlled) {
      if (!isOpen && onClose) onClose(false);
      return;
    }
  ...
  const generatePayslipPDF = (payslipData) => {
    ...
    doc.text("Tel: (123) 456-7890 | Email: payroll@company.com", 105, 36, {
      align: "center",
    });
    ...
    doc.text(`Employee: ${payslipData.employee_name || "John Doe"}`, 20, 55);
    doc.text(`Employee ID: ${payslipData.employee_id || "EMP001"}`, 20, 62);
    doc.text(`Position: ${payslipData.position || "Developer"}`, 20, 69);

    // Payslip Period
    doc.text(`Period: ${payslipData.month} ${payslipData.year}`, 130, 55);
    doc.text(`Payment Date: ${new Date().toLocaleDateString()}`, 130, 62);
    doc.text(`Status: ${payslipData.status}`, 130, 69);
    ...
    doc.text(`€${Number(payslipData.amount).toFixed(2)}`, 160, 95, {
      align: "right",
    });

    // Allowances
    const housingAllowance = Number(payslipData.amount) * 0.1;
    const transportAllowance = 200.0;
    const mealAllowance = 150.0;
    ...
    // Example deductions
    const tax = Number(payslipData.amount) * 0.15;
    const insurance = 120.0;
    const pension = Number(payslipData.amount) * 0.05;
    ...
    // Total Calculations
    const totalEarnings =
      Number(payslipData.amount) + housingAllowance + transportAllowance + mealAllowance;
    ...
    doc.text("This is a computer-generated document. No signature required.", 105, 250, {
      align: "center",
    });
    doc.text("Confidential - For employee use only", 105, 256, { align: "center" });
    doc.text(
      `Payslip ID: ${payslipData.id} | Generated: ${new Date().toLocaleString()}`,
      105,
      262,
      { align: "center" }
    );
  ...
  // Handle view payslip
  const handleViewPayslip = (payslipToView) => {
    try {
      setSelectedPayslip(payslipToView);
      setIsLoading(true);
      setPdfError(false);

      // Generate PDF
      const url = generatePayslipPDF(payslipToView);
      setPdfUrl(url);
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            {selectedPayslip && (
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <span>Amount: €{Number(selectedPayslip.amount)?.toLocaleString()}</span>
                <span
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Amount: €{Number(p.amount)?.toLocaleString()} | Status: {p.status}
                    </p>
                  </div>
        ...
        {selectedPayslip && (
          <div className="flex-1 flex flex-col min-h-0 p-6 pt-4">
            <div className="flex-1 border rounded-lg relative min-h-0 bg-gray-100">
              ...
              {pdfError ? (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8">
                  ...
                  <p className="text-sm text-center mb-4">The payslip PDF could not be loaded.</p>
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <Button onClick={handleDownload} disabled={!pdfUrl || pdfError}>
                <Download className="w-4 h-4 mr-2" />
                Download PDF
              </Button>
            </div>
